...
def init_asteroids():
    return init_asteroids_granvik(10000, True)
...
def callbackF(xk, convergence=0):
    print(f"iter: {len(xk['x_iters'])}, semi-major: {xk['x'][0]:.4}, eccentricity: {xk['x'][1]:.4}, spread: {xk['x'][2]:.4}, completeness: {1-xk['fun']:.3%}")
    return 0
...
if __name__ == '__main__':
    ...
    starttime = time.time()
    results['ET'] = forest_minimize(do_survey, bounds, callback=callbackF, base_estimator='ET', random_state=9, n_jobs=8, n_calls=10)
    print(f"ExtraTrees done in:   {time.time() - starttime:.2}")
    results['ET']['time'] = time.time() - starttime
    f = open('opt_tir_5_granvik_10000_SKOPT.pickle', "wb+")
    pickle.dump(results, f)
    f.close()


    starttime = time.time()
    results['Gauss'] = gp_minimize(do_survey, bounds, callback=callbackF, random_state=9, n_jobs=8, n_calls=10)
    print(f"Gauss done in:   {time.time() - starttime:.2}")
    results['Gauss']['time'] = time.time() - starttime
    f = open('opt_tir_5_granvik_10000_SKOPT.pickle', "wb+")
    pickle.dump(results, f)
    f.close()
    
    starttime = time.time()
    results['RF'] = forest_minimize(do_survey, bounds, callback=callbackF, base_estimator='RF', random_state=9, n_jobs=8, n_calls=10)
    print(f"RandomForest done in:   {time.time() - starttime:.2}")
    results['RF']['time'] = time.time() - starttime
    f = open('opt_tir_5_granvik_10000_SKOPT.pickle', "wb+")
    pickle.dump(results, f)
    f.close()
