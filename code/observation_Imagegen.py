...
for i in range(0, 360, 90):
    for j in range(0, 181, 45):
        ...
        signalTarget = fluxTarget * aperture * quantumEff *\
            transmittance*integrationTime / coulomb
            
        I_0 = 0.9 * signalTarget    # BEUN intensity in center pixel
        sigma = 4/2.355             # More beun
        
        ratio = (signalTarget * straddleFac) /\
              sqrt(readNoise + darkNoise + signalBG + signalTarget)
        image = np.zeros((65, 65))
        
        offset_x = np.random.randint(10, 55)
        offset_y = np.random.randint(10, 55)
        
        for p in range(65):
            for q in range(65):
                image[p, q] = poisson(I_0 * exp(-1*(abs(p-offset_x)**2 + abs(q-offset_y)**2) / 
                                        2*sigma**2)) + \
