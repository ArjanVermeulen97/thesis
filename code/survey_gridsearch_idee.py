...
def satellite_positions(day, a_speed):
    '''Propagate satellite positions'''
    for sat in satellites.values():
        sat['M'] = theta_step(a_speed, sat['anomaly'], day)
...
if __name__ == '__main__':
    results = pd.DataFrame(columns = ['n', 'a', 'e', 'completeness', 'aspeed'])
    verbose = False
    df_asteroids = init_asteroids()
    eccentricity = 0
    for semi_major in np.arange(0.2, 2.0, 0.1):
        for a_speed in np.arange(0.2, 2.0, 0.1):
            for spread in np.arange(0.2, 0.25, 0.05):
                for n_sats in range(1, 6):
                    print(f"Satellites:   {n_sats}")
                    print(f"Semi-Major:   {semi_major}")
                    print(f"Eccentricity: {eccentricity}")
                    print(f"Spread:       {spread}")
                    print(f"Speed:        {a_speed}")
                    ...
                    for day in range(0, 1825, 21):
                        ...
                        satellite_positions(day, a_speed)
                        for sat in satellites.values():
    ...
    results.to_csv('results_1000_jpl_a_speed.csv')
