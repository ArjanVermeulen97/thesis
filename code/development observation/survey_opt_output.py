...
def callbackF(xk, convergence=1):
    print(f"semi-major: {xk[0]:.4}, eccentricity: 0.000, spread: {xk[1]:.4}, convergence: {convergence:.3%}")
    print("")
    return 0
...
def do_survey(x):
    semi_major, eccentricity, spread = x[0], 0, x[1]
    n_sats = 5
...
if __name__ == '__main__':
    bounds = [(0.1, 2.0), (0, 2*pi/5)]
    results = dict()
    starttime = time.time()
    results['DE'] = optimize.differential_evolution(do_survey, bounds, workers=-1, disp=True, callback=callbackF)
    print(f"DE done in:   {time.time() - starttime:.2}")
    results['DE']['time'] = time.time() - starttime
    starttime = time.time()
    f = open('opt_tir_5_jpl_1000_DE.pickle', "wb+")
