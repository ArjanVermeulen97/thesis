...
def do_survey(x):
    ...
    for day in range(0, 365, 21):
        ...
        print(f"{NEval}: {semi_major:.2}, {eccentricity:.2}, {spread:.2}, {completeness[-1]:.2%}, {day}")
...
if __name__ == '__main__':
    NEval = 0
    bounds = [(0.1, 2.5), (0, 0.9), (0, 2*pi/5)]
    results = dict()
    starttime = time.time()
    
    results['shgo'] = optimize.shgo(do_survey, bounds, callback=callbackF, options={'f_min': 0.01, 'f_tol': 0.01})
    ...
    results['DA'] = optimize.dual_annealing(do_survey, bounds, callback=callbackF, options={'f_min': 0.01, 'f_tol': 0.01})
    ...
    results['DE'] = optimize.differential_evolution(do_survey, bounds, callback=callbackF, options={'f_min': 0.01, 'f_tol': 0.01})
    ...
    results['BH'] = optimize.basinhopping(do_survey, bounds, callback=callbackF, options={'f_min': 0.01, 'f_tol': 0.01})
